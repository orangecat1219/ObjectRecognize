...
def run_sift_gridsearch(selected_classes, max_per_class, k_list, kernel_list, C_list,
    ...
    for k, kernel, C in product(k_list, kernel_list, C_list):
        ...
        color_features = extract_color_histograms(images)
        combined_features = np.hstack([bow_features, color_features])

        X_train, X_test, y_train, y_test = train_test_split(combined_features, labels, test_size=0.2, stratify=labels)
    ...
    best_folder = f"best_SIFT_CIFAR10_color_k-{best_model_data['k']}_kernel-{best_model_data['kernel']}_C-{best_model_data['C']}"
